// The bots: eight ways of playing this aeroplane badly, and one of them on purpose.
//
// These were written for tools/simulate.js, which flies thousands of flights in a sandbox and
// complains about anything that throws, and they live here rather than there because the title
// screen flies one too. The cabin behind the boarding pass is not a loop of video and it is not
// a scripted demo: it is the real simulation with one of the bots at the controls, playing
// slowly enough to watch, and the fire it is losing to is the fire the player is about to lose to.
//
// A bot is `(PRS, S, list) -> entry | null`, where `list` is what `actions.available` says can be
// done right now and the entry is the one it takes. Nothing here reaches into the world directly:
// a bot chooses from what the player could have chosen from, which is what makes the balance
// table in simulate.js mean anything.
//
// The bots' coin is kept well away from the game's. The flight's dice are cast at boarding from
// its seed and never touched again, so a bot cannot shift them.
(function (global) {
    "use strict";
    ...
    PRS.bots = { BOTS, setCoin, names: () => Object.keys(BOTS) };
